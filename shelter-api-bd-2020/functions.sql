...
--TODO - should check for sex correctnes
CREATE OR REPLACE FUNCTION add_adoption(c_id INT, g_id INT, sex "plec", min_age INT, max_age INT, rasy int[]) RETURNS int AS'
    ...
    BEGIN
        ...
        LOOP
            FOR animal in select * from "Zwierzeta" where rasa_id=rasy[i] and new."Z_plec" = "A_plec"
            LOOP 
        ...
        FOR i IN 1 .. array_upper(rasy, 1)
        LOOP
            insert into "Ad_Ra" ("adopcja_id", "rasa_id") values (a_id, i);
        END LOOP;
        RETURN 1;
    END;
' LANGUAGE 'plpgsql';
